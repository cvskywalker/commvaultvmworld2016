if platform?("windows")
	template "C:\\#{node['commvaultvmworld']['xml']['win_fs_install']}" do
		source "#{node['commvaultvmworld']['xml']['win_fs_install']}.erb"
	end
...
# If I'm not windows, I must be non-windows
	...
	template "/tmp/#{node['commvaultvmworld']['xml']['linux_fs_install']}" do
		source "#{node['commvaultvmworld']['xml']['linux_fs_install']}.erb"
	end
	...
	if node.default['commvaultvmworld']['buildmethod'] == "s3"
		...
		bash "install_module" do
		    ...
		    /tmp/cv_install/#{node['commvaultvmworld']['buildvagrant']['Linux_x8664_folder']}/DownloadPackageLocation/silent_install --xmlfile /tmp/#{node['commvaultvmworld']['xml']['linux_fs_install']}
		...
		bash "install_module" do
		    user 'root'
		    code <<-EOH
		    /tmp/cv_install/#{node['commvaultvmworld']['buildlocal']['Linux_x8664_folder']}/DownloadPackageLocation/silent_install --xmlfile /tmp/#{node['commvaultvmworld']['xml']['linux_fs_install']}
		    EOH
